...
void ekbaar() {
    int n, k;
    cin >> n >> k;
    vi a(n + 1), last(k + 1, -1); // last[i] = last occurence of color i
    for (int i = 1; i <= n; i++) cin >> a[i];

    map<int, pair<int, int>> mp; // {color, {second max diff,max diff}}
    ...
    for (int i = 1; i <= k; i++) {
        ...
        else if (jump > mp[i].ff) {
            mp[i].ff = jump;
        }
    }
    int ans = 1e9;
    for (auto i : mp) {
        int jump = max(i.ss.ff, (i.ss.ss / 2));
        ans = min(ans, jump);
    }
    prn(ans);
}
