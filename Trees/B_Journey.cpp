...
void dfs(int cur, int par, vvi& adj, double pathlen, double prob) {
    if (adj[cur].size() == 1 && cur != 1) {
        total_path_len += pathlen * prob;
        return;
    }
    for (auto& child : adj[cur]) {
        if (child != par) {
            dfs(child, cur, adj, pathlen + 1, prob / (adj[cur].size() - (par != 0)));
        }
    }
}
void ekbaar() {
    ...
    total_path_len = 0;
    dfs(1, 0, adj, 0, 1);
    cout << fixed << setprecision(10) << total_path_len << endl;
}
