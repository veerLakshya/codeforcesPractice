...
ll roadhelper(ll curr, ll parent, vvll& edges, vll& roads, vll& subTreeSum) {
    if (roads[curr] == 1) subTreeSum[curr]++;
    for (auto child : edges[curr]) {
        if (child == parent) continue;
        subTreeSum[curr] += roadhelper(child, curr, edges, roads, subTreeSum);
    }
    return subTreeSum[curr];
}

void ekbaar() {
    // *- A node X is an answer, if it is connected to an unrepaired road and there is no unrepaired road in its subtree
    inll(n);
    vvll adj(n + 1);
    vll a(n + 1, 0);
    repn(i, 1, n - 1) {
    ...
    vll subTreesum(n + 1, 0);
    roadhelper(1, 0, adj, a, subTreesum);
    vi ans;
    for (int i = 1; i <= n; i++) {
        if (subTreesum[i] == 1) ans.pb(i);
    }
    prn(sz(ans));
    printv(ans);
}
