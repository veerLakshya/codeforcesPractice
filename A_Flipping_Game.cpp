...
void ekbaar() {
    ...
    // dp[i][j] = max no. of ones in array after i flip from i to j 

    for (int i = 1; i <= n; i++) {
        for (int j = i; j <= n; j++) {
            // dp[i][j] = zeroes inside i and j and outside i and j
            ll zeroes = pre0[j] - pre0[i - 1];
            ll ones = pre1[n] - pre1[j] + pre1[i - 1];
            // dbg(i, j, zeroes, ones);
            dp[i][j] = zeroes + ones;
        }
    }
