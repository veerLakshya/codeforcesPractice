...
void ekbaar(ll testcase) {
    ...
    vll result(n + 1, 0);
    auto dfs = [&](ll node, ll prefix, vll& suffix, auto& self)->void {
        for (auto nbr : adj[node]) {
            ll newprefix = prefix + a[nbr];
            ll lastsuffix = (!sz(suffix) ? 0 : suffix.back()) + b[nbr];

            suffix.pb(lastsuffix);
            ll l = 0, r = sz(suffix) - 1, ans = 0;
            while (l <= r) {
                ll mid = (l + r) / 2;
                if (suffix[mid] <= newprefix) {
                    ans = mid + 1;
                    l = mid + 1;
                }
                else r = mid - 1;
            ...
            result[nbr] = ans;
            self(nbr, newprefix, suffix, self);
            suffix.ppb();
        }
        };
