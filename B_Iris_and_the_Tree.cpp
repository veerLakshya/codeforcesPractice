...
/*~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-CODE~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-*/
vvi adj, belongsto; // belongsto tells this belongs to which two dis(i,j)
stack<int> st;
vector<int> unk;
...
void ekbaar(ll testcase) {
    ...
    rep(i, 1, n) sort(all(adj[i])); // to move to smallest value first during dfs

    dfs(1);

    while (sz(st)) {
        unk[1]++;
        belongsto[st.top()].pb(1);
        st.pop();
    }
