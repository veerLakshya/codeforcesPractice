...
void ekbaar(ll testcase) {
    vll a(6); cin >> a; // a b, x1 x2, x3 x4
    // dbg(a);
    vll dx = { -1,1,-1,1 }, dy = { -1,-1,1,1 };
    set<pll> king, queen;
    rep(i, 0, 3) {
        king.insert({ a[2] + (dx[i] * a[0]), a[3] + (dy[i] * a[1]) });
        king.insert({ a[2] + (dx[i] * a[1]), a[3] + (dy[i] * a[0]) });

        queen.insert({ a[4] + (dx[i] * a[1]), a[5] + (dy[i] * a[0]) });
        queen.insert({ a[4] + (dx[i] * a[0]), a[5] + (dy[i] * a[1]) });
    }
    // dbg(king);
    int ans = 0;
    for (auto i : king) {
        if (queen.find(i) != queen.end())ans++;
    }
